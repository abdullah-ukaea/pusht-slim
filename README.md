...
- **Observation encoding** — frames are cropped to 84×84 (random in train, center in eval) and passed through a pretrained DINOv2 ViT-S/14, fine-tuned end-to-end. Its 6×6 patch grid is mean-pooled for the global image condition and also retained as 36 dense visual tokens. The 2-DoF robot state goes through a small MLP.
- **Action generation** — one zero-gated cross-attention block lets the 16 action tokens query all 36 visual tokens before the existing DiT stack. The DiT retains its global adaLN conditioning (mean-pooled image + state + flow time) and predicts the velocity field of a flow from Gaussian noise to a 16-step action chunk. Inference is 10 Euler steps.
